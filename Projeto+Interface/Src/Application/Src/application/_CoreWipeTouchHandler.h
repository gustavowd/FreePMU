...
#ifndef _CoreWipeTouchHandler_H
#define _CoreWipeTouchHandler_H
...
/* Forward declaration of the class Core::WipeTouchHandler */
#ifndef _CoreWipeTouchHandler_
  EW_DECLARE_CLASS( CoreWipeTouchHandler )
#define _CoreWipeTouchHandler_
#endif
...
/* The class Core::WipeTouchHandler provides a special kind of a view able to react 
   to touch screen events and to recognize wipe gestures. Each time the user taps 
   inside the area of the handler view and drags the finger in a direction the handler 
   sends a signal to a slot method corresponding to this direction. For example, 
   if there is a slot method assigned to the property @OnRight, the handler will 
   send a signal to this slot method as soon as the user has touched the handler 
   and dragged his finger to the right. The intention of this handler is to cover 
   the internal aspects of the cursor event handling.
   The handler provides several variables useful to query e.g. the current handler 
   state (@Down), or the recognized direction (@Direction). These variables can 
...
EW_DEFINE_FIELDS( CoreWipeTouchHandler, CoreRectView )
  EW_VARIABLE( activeFinger,    XInt32 )
  EW_VARIABLE( HittingPos,      XPoint )
  EW_PROPERTY( OnBottomRight,   XSlot )
  EW_PROPERTY( OnTopRight,      XSlot )
  EW_PROPERTY( OnBottomLeft,    XSlot )
  EW_PROPERTY( OnTopLeft,       XSlot )
  EW_PROPERTY( OnBottom,        XSlot )
  EW_PROPERTY( OnRight,         XSlot )
  EW_PROPERTY( OnEnd,           XSlot )
  EW_PROPERTY( OnTop,           XSlot )
  EW_PROPERTY( OnLeft,          XSlot )
  ...
  EW_PROPERTY( Threshold,       XFloat )
  EW_VARIABLE( Direction,       XEnum )
  EW_VARIABLE( Down,            XBool )
  EW_VARIABLE( Inside,          XBool )
  EW_VARIABLE( AutoDeflected,   XBool )
  EW_VARIABLE( active,          XBool )
EW_END_OF_FIELDS( CoreWipeTouchHandler )

/* Virtual Method Table (VMT) for the class : 'Core::WipeTouchHandler' */
EW_DEFINE_METHODS( CoreWipeTouchHandler, CoreRectView )
  EW_METHOD( initLayoutContext, void )( CoreRectView _this, XRect aBounds, CoreOutline 
    aOutline )
  EW_METHOD( GetRoot,           CoreRoot )( CoreView _this )
  EW_METHOD( Draw,              void )( CoreWipeTouchHandler _this, GraphicsCanvas 
    aCanvas, XRect aClip, XPoint aOffset, XInt32 aOpacity, XBool aBlend )
  EW_METHOD( HandleEvent,       XObject )( CoreWipeTouchHandler _this, CoreEvent 
    aEvent )
  EW_METHOD( CursorHitTest,     CoreCursorHit )( CoreWipeTouchHandler _this, XRect 
...
EW_END_OF_METHODS( CoreWipeTouchHandler )
