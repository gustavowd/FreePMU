...
static void _AddValues(void) {
  ...
  for (i = 0; i < GUI_COUNTOF(_aColor); i++) {
    ...
    //valor.x = j;
    valor.y = _aValue[i]+30;
    GRAPH_DATA_YT_AddValue(_ahData[i], _aValue[i]+30);
  }
}

BRTOS_TIMER TouchTimer;
BRTOS_TIMER GraphTimer;

TIMER_CNT UpdateTouch(void)
{
	TM_EMWIN_UpdateTouch();
	return 50;
}

TIMER_CNT UpdateGraph(void)
{
	_AddValues();
	GUI_X_SignalEvent();
	return 5;
...
GRAPH_SCALE_Handle yScale;
GRAPH_SCALE_Handle xScale;
TEXT_Handle text;
...
void EmWinTask(void *param)
{
	...
	LCD_SetVSize(320, 240);
	LCD_SetVSizeEx(0, 320, 240);

	OSTimerInit(1024,3);
	OSTimerSet(&TouchTimer,UpdateTouch,50);
	...
	*/
    ...
    GRAPH_SetBorder(hGraph, 25, 5, 5, 15);
	GRAPH_SetColor(hGraph, 0x00202020, GRAPH_CI_GRID);

	/* Create scale for graph */
	yScale = GRAPH_SCALE_Create(3, GUI_TA_LEFT, GRAPH_SCALE_CF_VERTICAL, 25);
	GRAPH_SCALE_SetOff(yScale, 100);
	GRAPH_SCALE_SetTextColor(yScale, GUI_BLUE);
	/* Attach it to graph */
	GRAPH_AttachScale(hGraph, yScale);

	xScale = GRAPH_SCALE_Create(230, GUI_TA_HCENTER, GUI_TA_HCENTER, 50);
	GRAPH_SCALE_SetTextColor(xScale, GUI_BLUE);
	GRAPH_SCALE_SetFactor(xScale, 4);
	/* Attach it to graph */
	GRAPH_AttachScale(hGraph, xScale);

	text = TEXT_Create(290, 225, 30, 15, GUI_ID_TEXT0, WM_CF_SHOW, "500us",GUI_TA_LEFT | GUI_TA_TOP);
	TEXT_SetFont(text, &GUI_Font13_ASCII);
	TEXT_SetTextColor(text,GUI_RED);
	TM_EMWIN_Exec();
    ...
    for (i = 0; i < GUI_COUNTOF(_aColor); i++) {
      _aValue[i] = rand() % 180;
      _ahData[i] = GRAPH_DATA_YT_Create(_aColor[i], 290, 0, 0);
      GRAPH_DATA_YT_SetAlign(_ahData[i], GRAPH_ALIGN_LEFT);
      GRAPH_AttachData(hGraph, _ahData[i]);
    }

	/* Create button with GUI_ID_OK ID number */
    hButton = BUTTON_Create(240, 5, 80, 40, GUI_ID_BUTTON0, WM_CF_SHOW);
    hB1 = BUTTON_Create(240, 50, 80, 40, GUI_ID_BUTTON1, WM_CF_SHOW);
    hB2 = BUTTON_Create(240, 95, 80, 40, GUI_ID_BUTTON2, WM_CF_SHOW);
    hB3 = BUTTON_Create(240, 140, 80, 40, GUI_ID_BUTTON3, WM_CF_SHOW);
    hB4 = BUTTON_Create(240, 185, 80, 40, GUI_ID_BUTTON4, WM_CF_SHOW);
    //hB4 = BUTTON_CreateEx(240, 195, 80, 40, WM_HBKWIN, WM_CF_SHOW, 0, GUI_ID_BUTTON4);
    BUTTON_SetFont(hButton, &GUI_Font13HB_ASCII);
    ...
    BUTTON_SetText(hButton, "Continue...");
    BUTTON_SetText(hB1, "Green\n\rON");
    BUTTON_SetText(hB2, "Green\n\rOFF");
    BUTTON_SetText(hB3, "Red\n\rON");
    BUTTON_SetText(hB4, "Red\n\rOFF");
    GRAPH_SetBorder(hGraph, 25, 5, 90, 15);
    BUTTON_SetTextAlign(hB1,GUI_TA_HCENTER | GUI_TA_VCENTER);
    BUTTON_SetTextAlign(hB2,GUI_TA_HCENTER | GUI_TA_VCENTER);
    BUTTON_SetTextAlign(hB3,GUI_TA_HCENTER | GUI_TA_VCENTER);
    BUTTON_SetTextAlign(hB4,GUI_TA_HCENTER | GUI_TA_VCENTER);
    ...
    OSTimerSet(&GraphTimer,UpdateGraph,5);
